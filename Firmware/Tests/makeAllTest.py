...
def find_tests(tests_dir: Path):
    ...
    for cfile in tests_dir.glob("*.c"):
        # remove .c from all files
        test_name = cfile.name.replace(".c","")
...
def main():
    ...
    # Where tests live relative to top-level git repo directory
    tests_dir = Path("Firmware/Tests")
    tests = find_tests(git_dir / tests_dir)

    # Wherever software Makefile lives relative to top-level git repo directory
